...
package org.nameless.settings.preference.colorpicker;
...
public class ColorPickerView extends View {
    ...
    private void drawSatValPanel(Canvas canvas) {
        ...
        canvas.drawRect(mDrawingRect.left,
            mDrawingRect.top,
            mSatValRect.right + BORDER_WIDTH_PX,
            mSatValRect.bottom + BORDER_WIDTH_PX,
            mBorderPaint);

        if (mValShader == null) {
            mValShader = new LinearGradient(mSatValRect.left,
                    mSatValRect.top,
                    mSatValRect.left,
                    mSatValRect.bottom,
        ...
        mSatShader = new LinearGradient(mSatValRect.left,
                mSatValRect.top,
                mSatValRect.right,
                mSatValRect.top,
                0xffffffff, rgb,
        ...
        final ComposeShader composeShader = new ComposeShader(
            mValShader, mSatShader, PorterDuff.Mode.MULTIPLY);
        mSatValPaint.setShader(composeShader);
    ...
    private void drawHuePanel(Canvas canvas) {
        ...
        mBorderPaint.setColor(mBorderColor);
        canvas.drawRect(mHueRect.left - BORDER_WIDTH_PX,
                mHueRect.top - BORDER_WIDTH_PX,
                mHueRect.right + BORDER_WIDTH_PX,
                mHueRect.bottom + BORDER_WIDTH_PX,
                mBorderPaint);
    ...
    private void drawAlphaPanel(Canvas canvas) {
        ...
        mBorderPaint.setColor(mBorderColor);
        canvas.drawRect(mAlphaRect.left - BORDER_WIDTH_PX,
                mAlphaRect.top - BORDER_WIDTH_PX,
                mAlphaRect.right + BORDER_WIDTH_PX,
                mAlphaRect.bottom + BORDER_WIDTH_PX,
                mBorderPaint);
        ...
        mAlphaShader = new LinearGradient(
                mAlphaRect.left,
                mAlphaRect.top,
                mAlphaRect.right,
                mAlphaRect.top,
        ...
        if (mAlphaSliderText != null && mAlphaSliderText.length() > 0) {
            canvas.drawText(mAlphaSliderText, mAlphaRect.centerX(),
                mAlphaRect.centerY() + 4 * mDensity, mAlphaTextPaint);
        }
        ...
        r.top = mAlphaRect.top - RECTANGLE_TRACKER_OFFSET;
        r.bottom = mAlphaRect.bottom + RECTANGLE_TRACKER_OFFSET;

        canvas.drawRoundRect(r, 2, 2, mHueTrackerPaint);
    }

    private Point hueToPoint(float hue) {
        final float height = mHueRect.height();
        final Point p = new Point();
        p.y = (int) (height - (hue * height / 360f) + mHueRect.top);
        p.x = (int) mHueRect.left;
    ...
    private Point alphaToPoint(int alpha) {
        final float width = mAlphaRect.width();
        final Point p = new Point();
        p.x = (int) (width - (alpha * width / 0xff) + mAlphaRect.left);
        p.y = (int) mAlphaRect.top;
    ...
    private int pointToAlpha(int x) {
        final int width = (int) mAlphaRect.width();
    ...
    private void setUpHueRect() {
        final float left = mDrawingRect.right - HUE_PANEL_WIDTH + BORDER_WIDTH_PX;
        final float top = mDrawingRect.top + BORDER_WIDTH_PX;
    ...
    private void setUpAlphaRect() {
        ...
        final float left = mDrawingRect.left + BORDER_WIDTH_PX;
        final float top = mDrawingRect.bottom - ALPHA_PANEL_HEIGHT + BORDER_WIDTH_PX;
        final float bottom = mDrawingRect.bottom - BORDER_WIDTH_PX;
        final float right = mDrawingRect.right - BORDER_WIDTH_PX;
