...
public class CustomSeekBarPreference extends Preference implements
        SeekBar.OnSeekBarChangeListener,
        View.OnClickListener, View.OnLongClickListener {

    protected static final String TAG = "CustomSeekBarPreference";
    ...
    public CustomSeekBarPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        try {
            final String newInterval = attrs.getAttributeValue(SETTINGS_NS, "interval");
            if (newInterval != null) {
    ...
    public void onBindViewHolder(PreferenceViewHolder holder) {
        ...
        try {
            ...
            final ViewGroup newContainer = (ViewGroup) holder.findViewById(R.id.seekbar);
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        final int newValue = getLimitedValue(mMinValue + (progress * mInterval));
        if (mTrackingTouch && !mContinuousUpdates) {
            mTrackingValue = newValue;
            updateValueViews();
            if (mMinValue != mMaxValue) {
                doHapticFeedback((float) (newValue - mMinValue) / (mMaxValue - mMinValue));
            } else {
                doHapticFeedback(1.0f);
            }
    ...
    public void onClick(View v) {
        ...
        if (mMinValue != mMaxValue) {
            doHapticFeedback((float) (mValue - mMinValue) / (mValue - mMinValue));
        } else {
            doHapticFeedback(1.0f);
        }
    }
    ...
    public boolean onLongClick(View v) {
        ...
        if (id == R.id.reset) {
            ...
            setValue(mMaxValue - mMinValue > mInterval * 2 && mMaxValue + mMinValue < mValue * 2 ?
                    Math.floorDiv(mMaxValue + mMinValue, 2) : mMinValue, true);
        } else if (id == R.id.plus) {
            setValue(mMaxValue - mMinValue > mInterval * 2 && mMaxValue + mMinValue > mValue * 2 ?
                    -1 * Math.floorDiv(-1 * (mMaxValue + mMinValue), 2) : mMaxValue, true);
        }
        return true;
    }
    ...
    private void doHapticFeedback(float progress) {
