...
public class ConstraintsHelper {
    ...
    public ConstraintsHelper(Context context, AttributeSet attrs, Preference pref) {
        ...
        TypedArray a = context.getResources().obtainAttributes(attrs,
                R.styleable.CustomSelfRemovingPreference);
        try {
            mSummaryMinLines = a.getInteger(R.styleable.CustomSelfRemovingPreference_minSummaryLines, -1);
            final String replacesKey = a.getString(R.styleable.CustomSelfRemovingPreference_replacesKey);
            if (replacesKey != null) {
        ...
        if (DEBUG) Log.d(TAG, "construct key=" + mPref.getKey() + " available=" + mAvailable);
    }
    ...
    private PreferenceGroup getParent(PreferenceGroup root, Preference preference) {
        for (int i = 0; i < root.getPreferenceCount(); i++) {
            ...
            if (PreferenceGroup.class.isInstance(p)) {
                final PreferenceGroup parent = getParent((PreferenceGroup) p, preference);
    ...
    private void checkIntent() {
        final Intent i = mPref.getIntent();
        if (i != null) {
    ...
    private boolean checkConstraints() {
        ...
        try {
            ...
            boolean rOwner = a.getBoolean(R.styleable.CustomSelfRemovingPreference_requiresOwner, false);
            if (rOwner && UserHandle.myUserId() != UserHandle.USER_OWNER) {
                return false;
            }

            // Check if a specific package is installed
            String rPackage = a.getString(R.styleable.CustomSelfRemovingPreference_requiresPackage);
            if (rPackage != null) {
                final boolean negated = isNegated(rPackage);
                if (negated) {
                    rPackage = rPackage.substring(1);
                }
                final boolean available = isPackageInstalled(mContext, rPackage, false);
            ...
            String rAction = a.getString(R.styleable.CustomSelfRemovingPreference_requiresAction);
            if (rAction != null) {
                final boolean negated = isNegated(rAction);
                if (negated) {
                    rAction = rAction.substring(1);
                }
                final boolean available = resolveIntent(mContext, rAction);
                if (available == negated) {
                    return false;
                }
            }
            ...
            String rFeature = a.getString(R.styleable.CustomSelfRemovingPreference_requiresFeature);
            if (rFeature != null) {
                final boolean negated = isNegated(rFeature);
                if (negated) {
                    rFeature = rFeature.substring(1);
                }
                final boolean available = hasSystemFeature(mContext, rFeature);
                if (available == negated) {
                    return false;
                }
            }
            ...
            String rProperty = a.getString(R.styleable.CustomSelfRemovingPreference_requiresProperty);
            if (rProperty != null) {
                final boolean negated = isNegated(rProperty);
                if (negated) {
                    rProperty = rProperty.substring(1);
                }
                String value = SystemProperties.get(rProperty);
                final boolean available = value != null && Boolean.parseBoolean(value);
                if (available == negated) {
                    return false;
                }
            }
            ...
            TypedValue tv = a.peekValue(R.styleable.CustomSelfRemovingPreference_requiresConfig);
            if (tv != null && tv.resourceId != 0) {
                if (tv.type == TypedValue.TYPE_STRING &&
                        mContext.getResources().getString(tv.resourceId) == null) {
                    return false;
                }
                if (tv.type == TypedValue.TYPE_INT_BOOLEAN && tv.data == 0) {
                    return false;
                }
                if (tv.type == TypedValue.TYPE_INT_DEC) {
                    final int mask = a.getInt(
                            R.styleable.CustomSelfRemovingPreference_requiresConfigMask, -1);
